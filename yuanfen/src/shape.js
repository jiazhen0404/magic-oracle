...
function shape(d) {
  ...
  const oneWay = zl === 'a_ke_b' || zl === 'b_ke_a';    // 剋＝壓，最不平衡
  ...
  else if (sweet === 2)                     pattern = 'sweetUneven';// 雙合但位置不平
...
const NOW = {
  harmony:
    '你們這張盤少見的整齊。相處合得來，長線也對得上，而且兩個人的位置是平的——沒有誰一直在等對方。' +
    '這種狀況在曖昧期很好認：你們幾乎不用刻意找話題，也不太需要猜下一步。真正的風險反而是太舒服，' +
    '舒服到誰都不想打破現狀，然後一直停在這裡。',
  doubleClash:
    '你們這張盤有兩處是相反的。相處有摩擦，節奏也對不上——這代表你感覺到的那種「怎麼老是差一點」不是錯覺。' +
    '曖昧期遇到這種相反特別難處理，因為還沒有承諾可以撐著，一有卡點就容易冷掉。' +
    '但問題是具體的，這比說不上哪裡不對好處理。',
  mixed:
    '你們這張盤一邊合得來、一邊對不上，這是曖昧期最折磨人的一種。' +
    '有時候{B}的回應讓你覺得就是{B}了，有時候又冷到你懷疑自己想太多，而且這兩種常常在同一週出現。' +
    '你反覆判斷不出來，不是你敏感——是這段關係本來就有兩個面。',
  sweetUneven:
    '你們合得來，這件事算得很清楚。但兩個人的位置不平，有一方一直在配合。' +
    '在曖昧期這特別容易被忽略，因為合得來讓一切看起來很順，而配合的那一方也還沒累到想抱怨。',
  pressed:
    '你們這張盤有一處明顯的摩擦，加上位置不平——這兩件事會互相放大。' +
    '摩擦一出現，配合的那一方會先退，於是事情看起來過去了，其實只是被吞下去。' +
    '曖昧期這樣的退讓不會被看見，因為根本還沒有人有立場抗議。',
  blank:
    '你們這張盤很乾淨，沒有合也沒有沖，位置也是平的。這代表你們之間沒有天生的推力，也沒有天生的阻力。' +
    '會不會走到一起，幾乎完全取決於你們做了什麼——這種盤最誠實，也最沒有藉口。' +
    '曖昧要往前，得有人先做一件明確的事。',
  quiet:
    '你們這張盤平穩，沒有明顯的合也沒有明顯的沖。相處舒服，但少了那種非要見到不可的拉力。' +
    '這是曖昧期最常見、也最容易無疾而終的一種——不是因為不合適，是因為沒有人覺得需要著急。',
  onePoint:
    '你們這張盤有一個很明確的亮點，其餘平穩。這代表你們之間有一件事是特別對的，' +
    '而那件事往往就是你會一直想起{B}的原因——只是你可能說不出那是什麼。',
  oneSnag:
    '你們這張盤大致平順，但有一處卡點。這種狀況通常沒有大問題，' +
    '只是那一個地方會反覆出現，久了會讓你以為整件事都沒希望——其實只有那一處。',
  plain:
    '你們這張盤沒有特別強的訊號，這在合盤裡其實是多數。沒有天生的默契，也沒有天生的相衝，' +
...
const WEAK_LINE = {
  ...
  changdu:    '具體卡住的地方在走向。你們現在相處得還可以，但這段關係沒有天生的長度——它不會自己往前，需要有人先跨一步。'
};
const NO_WEAK = {
  flat:
    '這一項要誠實講：你們三個項目的分數很接近，盤上沒有一個明顯的短板。' +
...
const STRONG_LINE = {
  ...
  zhongliang: '你們有一個很好的底子：位置是平的。沒有人在偷偷承受，也沒有人在無意識地佔上風。這表示你們之間該講的話是講得出口的，不用一直猜。',
  changdu:    '你們有一個天生的好處：這段緣本身走得遠。就算現在還沒有結果，它也不容易斷——時間站在你們這邊。'
};
const DIM_CN = { wendu: '日常相處', zhongliang: '位置是平的', changdu: '長線走向' };
...
const NO_STRONG = {
  harmony: '要說最值得珍惜的，是這份整齊本身。能同時合得來、位置又平的組合不多，你們現在擁有的東西比你以為的稀有。',
  plain:   '你們沒有一項特別突出，但也沒有一項特別差——這種均衡本身就是資產。它代表這段關係不會因為某個弱點突然崩掉。',
  clash:   '就算算出來有對不上的地方，你們還是走到了現在，這件事本身就說明了一些什麼。願意為對方調整的意願，是盤上算不出來的。'
};
